...
for mt in range(m_trials):
	...
	for trial in range(n_trials):
		...
		for t in range(horizon):
			...
			n_state, reward = grid_env.grid_environment(state, action)
...
with open('data_Combinedmodel_full.npy', 'wb') as file:
    np.save(file, t_length)
    
with open('Beta_full.npy', 'wb') as file:
    np.save(file, Beta_plotting)
with open('Gamma_full.npy', 'wb') as file:
